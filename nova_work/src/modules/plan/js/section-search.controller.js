...
export default class SectionListController{
	...
	getSearchInfo() {
		...
		self.SectionService.searchSections(this.filter).then(function (res) {
	...
	openModal() {
		var self = this;
		this.CreateModal.openModal(true, PlanAddNewSection, modalCtrl, 'lg', this.modal).result.then(function (result) {
